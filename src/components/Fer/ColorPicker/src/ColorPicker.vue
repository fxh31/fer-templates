<template>
  <a-popover
    ...
    trigger="click"
    class="fer-color-picker"
    :class="[colorSize ? 'fer-color-picker--' + colorSize : '']"
    v-bind="$attrs"
    v-if="!colorDisabled">
    <template #content>
  ...
  <div
    class="fer-color-picker ant-color-picker__trigger is-disabled"
    :class="[colorSize ? 'fer-color-picker--' + colorSize : '']"
...
<script lang="ts">
  ...
  import SvPanel from './components/svPanel.vue'; // 颜色选择主画布
...
<style lang="less" scoped>
  ...
  // large 大小
  .fer-color-picker--large {
    width: 40px;
    height: 40px;
  }

  //small 大小
  .fer-color-picker--small {
    width: 32px;
    height: 32px;
  }
</style>
