...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'FerCheckbox', inheritAttrs: false });
  ...
  const getBindValue = computed(() => ({
    ...unref(attrs),
    class: unref(attrs).class ? `fer-${props.direction}-checkbox ${unref(attrs).class}` : `fer-${props.direction}-checkbox`,
  }));
