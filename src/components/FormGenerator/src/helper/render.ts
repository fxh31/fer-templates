...
export default defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    function buildDataObject(confClone, dataObject, formData) {
      ...
      if (['calculate', 'table', 'barcode', 'qrcode', 'popupSelect', 'popupTableSelect', 'autoComplete'].includes(ferKey)) {
        dataObject['formData'] = formData;
      }
      if (['table'].includes(ferKey)) {
        dataObject['relations'] = props.relations;
        dataObject['vModel'] = confClone.__vModel__;
        dataObject['ref'] = 'tableRef';
      }
      if (['relationForm', 'popupSelect'].includes(ferKey)) {
        dataObject['field'] = confClone.__config__.tableName
          ? confClone.__vModel__ + '_ferTable_' + confClone.__config__.tableName + (confClone.__config__.isSubTable ? '0' : '1')
    ...
    return () => {
      ...
      const ferKey = upperFirst(props.conf.__config__.ferKey) === 'Table' ? 'InputTable' : upperFirst(props.conf.__config__.ferKey);
