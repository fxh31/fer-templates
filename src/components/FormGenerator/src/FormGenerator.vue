<template>
  <div :class="`${prefixCls}`">
              ...
              <template #item="{ element }">
                <div class="components-item" :class="{ disabled: element.__config__.dragDisabled }" @click="addComponent(element)">
                  <div class="components-body">
                    ...
                    <BasicHelp text="不支持代码生成" v-if="element.__config__.ferKey === 'calculate'" />
                  </div>
                </div>
              </template>
            </draggable>
    ...
    <div class="center-board common-board">
      <div class="action-bar">
        <div class="action-bar-left">
          <div class="fer-device-switch">
            <div class="fer-device-switch-item" :class="{ 'fer-device-switch-item--active': showType === 'pc' }" @click="state.showType = 'pc'">
              <a-tooltip title="PC">
                <i class="icon-ym icon-ym-pc" />
              </a-tooltip>
            </div>
            <div class="fer-device-switch-item" :class="{ 'fer-device-switch-item--active': showType === 'app' }" @click="state.showType = 'app'">
              <a-tooltip title="APP">
    ...
    <PreviewModal @register="registerPreviewModal" :formConf="formConf" @cancel="handlePreviewClose" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  function shouldClone(_to, _from, target, _event, conf) {
    ...
    if (conf.__config__.ferKey === 'table') {
      if (!targetConf) return false;
      if (noTableAllowList.includes(targetConf.__config__.ferKey) || state.leftTabActiveKey === '2') {
        return false;
      }
  ...
  function createIdAndKey(item, parent: Nullable<GenItem> = null) {
    ...
    if (config.layout === 'colFormItem') {
      if (noVModelList.indexOf(config.ferKey) < 0 || item.isStorage) {
        item.__vModel__ = !generatorStore.getHasTable ? toggleVmodelCase(`${config.ferKey}Field${uuid}`) : '';
      }
      if (parent?.__vModel__ && parent?.__config__.ferKey === 'table') {
        item.__config__.parentVModel = parent.__vModel__;
      }
    } else if (config.layout === 'rowFormItem') {
      if (config.ferKey === 'table') {
        item.__vModel__ = toggleVmodelCase(`${config.ferKey}Field${uuid}`);
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config.layout === 'colFormItem' && (!noVModelList.includes(config.ferKey) || e.isStorage)) {
            if (!e.__vModel__) {
          ...
          if (config.ferKey === 'billRule') {
            if (config.ruleType === 2) {
              if (config.ruleConfig?.type === 1) {
                if (!config.ruleConfig?.digit) {
                  reject({ msg: '单据组件中位数不能为空', target: 1 });
                  break;
                }
                if (!config.ruleConfig?.startNumber) {
                  reject({ msg: '单据组件中起始值不能为空', target: 1 });
                  break;
                }
                const RegExp = /^[0-9]*[1-9]$/;
                if (!RegExp.test(config.ruleConfig?.startNumber)) {
                  reject({ msg: '单据组件中起始值只能输入数字且不能为0', target: 1 });
                  break;
                }
              }
            } else {
          ...
          if (config.ferKey === 'relationForm') {
            if (!e.modelId) {
              reject({ msg: '关联表单控件“关联功能”属性为必填项', target: 1 });
              break;
            }
            if (!e.relationField) {
              reject({ msg: '关联表单控件“回显字段”属性为必填项', target: 1 });
              break;
            }
          }
          if (config.ferKey === 'popupSelect') {
            if (!e.interfaceId) {
            ...
            if (!e.relationField) {
              reject({ msg: '弹窗选择控件“回显字段”属性为必填项', target: 1 });
              break;
            }
          }
          if (config.ferKey === 'popupTableSelect') {
            if (!e.interfaceId) {
            ...
            if (!e.relationField) {
              reject({ msg: '下拉表格控件“回显字段”属性为必填项', target: 1 });
              break;
            }
          }
          if (config.ferKey === 'autoComplete') {
            if (!e.interfaceId) {
          ...
          if (config.ferKey === 'uploadFile' || config.ferKey === 'uploadImg') {
          ...
          if (config.ferKey === 'link' || config.ferKey === 'iframe') {
      ...
      if (state.formConf.useBusinessKey && !state.formConf.businessKeyList?.length) {
        reject({ msg: `业务主键的规则设置中联合字段不能为空`, target: 1 });
      }
