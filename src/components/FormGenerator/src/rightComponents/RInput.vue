<template>
  ...
  <a-form-item label="前图标" v-show="showType === 'pc'">
    <fer-icon-picker v-model:value="activeData.prefixIcon" placeholder="请选择" />
  </a-form-item>
  <a-form-item label="后图标" v-show="showType === 'pc'">
    <fer-icon-picker v-model:value="activeData.suffixIcon" placeholder="请选择" />
  </a-form-item>
  ...
  <div v-for="(item, index) in activeData.__config__.regList" :key="index" class="reg-item">
    ...
    <a-form-item label="错误提示" style="margin-bottom: 0">
      <fer-i18n-input v-model:value="item.message" v-model:i18n="item.messageI18nCode" placeholder="请输入" />
    </a-form-item>
  </div>
  <div class="mt-10px">
    <a-dropdown>
