  ...
  <BasicModal
    ...
    <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" :model="dataForm" :rules="formRules" ref="formElRef" hideRequiredMark>
      <a-form-item label="按钮名称" name="label">
        <fer-i18n-input v-model:value="dataForm.label" v-model:i18n="dataForm.labelI18nCode" placeholder="请输入" :maxlength="10" />
      </a-form-item>
      <fer-group-title content="动作设置" :bordered="false" />
      ...
      <template v-if="dataForm.btnType == 1">
        <a-form-item label="选择表单" name="modelId">
          <fer-tree-select v-model:value="dataForm.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
        </a-form-item>
        ...
        <a-form-item label="弹窗类型" name="popupType" v-if="showType === 'pc'">
          <fer-select v-model:value="dataForm.popupType" placeholder="请选择" :options="popupTypeOptions" />
        </a-form-item>
        ...
        <a-table :data-source="dataForm.formOptions" :columns="formOptionsColumns" size="small" :pagination="false">
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.key === 'field'">
              <fer-select v-model:value="record.field" placeholder="请选择" :options="fieldOptions" showSearch @dropdownVisibleChange="visibleChange" />
            </template>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'relationField'">
                ...
                <template v-else-if="record.sourceType == 2">
                  <fer-input-number v-if="['int', 'decimal'].includes(record.dataType)" v-model:value="record.relationField" placeholder="请输入" allowClear />
                  <fer-date-picker
...
<script lang="ts" setup>
  ...
  function getFormFieldsOptions(drawingList) {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && !noAllowFormFieldsList.includes(data.__config__.ferKey)) {
        list.push({
          id: data.__vModel__,
