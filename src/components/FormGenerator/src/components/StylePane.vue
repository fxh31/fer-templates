<template>
  <a-form :colon="false" layout="vertical" class="right-board-form">
    <a-form-item label="多端显示">
      <fer-checkbox v-model:value="activeData.__config__.visibility" :options="getVisibilityOptions" />
    </a-form-item>
    <template v-if="config.isSubTable && showType === 'pc'">
      <a-divider class="!mt-0">表格布局配置</a-divider>
      <a-form-item label="冻结方式">
        <fer-radio v-model:value="activeData.__config__.tableFixed" :options="fixedOptions" optionType="button" button-style="solid" class="right-radio" />
      </a-form-item>
      <a-form-item label="对齐方式">
        <fer-radio v-model:value="activeData.__config__.tableAlign" :options="alignOptions" optionType="button" button-style="solid" class="right-radio" />
      </a-form-item>
    ...
    <a-form-item v-if="!noWithList.includes(ferKey) && activeData.style && showType === 'pc'">
      <template #label>
        控件宽度
    ...
    <a-form-item>
      ...
      <fer-select v-model:value="activeData.__config__.className" multiple allowClear :options="getClassNameOptions" showSearch />
    </a-form-item>
  </a-form>
</template>
<script lang="ts" setup>
  ...
  const getVisibilityOptions = computed(() => (unref(ferKey) == 'iframe' ? iframeVisibilityOptions : visibilityOptions));
</script>
