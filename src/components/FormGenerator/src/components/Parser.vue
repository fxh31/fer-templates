<script lang="tsx">
  ...
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit, getParamList } from '@/utils/fer';
    ...
    setup(props, { expose, emit }) {
      ...
      const layouts = {
        colFormItem(element) {
          ...
          if (['groupTitle', 'divider', 'link', 'text'].includes(config.ferKey)) {
          ...
          const Item = (
            <render
              ...
              {...listeners}
              ref={config.ferKey === 'table' ? element.__vModel__ : undefined}
              relations={config.ferKey === 'table' ? state.relations : undefined}
            />
          );
          ...
          Object.keys(scheme.on).forEach(key => {
            ...
            listeners['on' + upperFirst(key)] = (...arg) => {
              if (key === 'change') {
                ...
                if (['popupSelect', 'relationForm'].includes(config.ferKey)) setTransferFormData(data, config);
        ...
        for (let key in currRelations) {
          if (key === field) {
            for (let i = 0; i < currRelations[key].length; i++) {
              ...
              const ferKey = config.ferKey;
              let defaultValue: any = null;
              if (
                ['checkbox', 'cascader', 'organizeSelect'].includes(ferKey) ||
                (['select', 'treeSelect', 'popupSelect', 'popupTableSelect', 'userSelect'].includes(ferKey) && e.multiple)
              ) {
                defaultValue = [];
              }
              if (vModel.includes('-')) {
      ...
      function comSet(field, prop, value) {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (item.__vModel__ && item.__vModel__ === prop) {
              switch (field) {
                ...
                case 'endTime':
                  ...
                  break;
                case 'options':
                  if (dyOptionsList.indexOf(item.__config__.ferKey) > -1) item.options = value;
                  break;
                default:
            ...
            if (item.__config__ && item.__config__.ferKey !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
      ...
      function initFormData(componentList) {
        ...
        componentList.forEach(cur => {
          ...
          if (cur.__config__.ferKey == 'table') return;
      ...
      function initRelationForm(componentList) {
        componentList.forEach(cur => {
          const config = cur.__config__;
          if (config.ferKey == 'relationFormAttr' || config.ferKey == 'popupAttr') {
            const relationKey = cur.relationField.split('_ferTable_')[0];
      ...
      function buildRules(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.ferKey !== 'table') buildRules(config.children);
        });
      }
      ...
      function buildOptions(componentList) {
        componentList.forEach(cur => {
          const config = cur.__config__;
          if (dyOptionsList.includes(config.ferKey)) {
          ...
          if (config.children && config.ferKey !== 'table') buildOptions(config.children);
        });
      }
      function buildRelations(componentList, relations) {
        componentList.forEach(cur => {
          const config = cur.__config__;
          if (dyOptionsList.indexOf(config.ferKey) > -1) {
          ...
          if (config.ferKey === 'userSelect' && ['dep', 'pos', 'role', 'group'].includes(cur.selectType)) {
