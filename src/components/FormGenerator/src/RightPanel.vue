    ...
    <div class="field-box" v-if="activeData?.__config__ && activeData.__config__.ferKey">
      <ScrollContainer style="height: 100%">
        <a-form :colon="false" layout="vertical" v-show="activeKey === 'field'" class="right-board-form">
          <a-form-item label="控件类型" v-if="!['tableGridTr', 'tableGridTd'].includes(activeData.__config__.ferKey)">
            <a-input v-model:value="getCompName" disabled />
          </a-form-item>
          <a-form-item label="控件用途" v-if="ferKey == 'calculate'">
            <fer-select v-model:value="activeData.isStorage" :options="storageTypeOptions" @change="onStorageChange" />
          </a-form-item>
          <div v-if="(activeData.__vModel__ !== undefined && !noVModelList.includes(ferKey)) || activeData.isStorage">
            <div v-if="getHasTable">
              <div v-if="!config.isSubTable">
                <a-form-item label="数据库表" v-if="ferKey !== 'table'">
                  <fer-select
                ...
                <a-form-item>
                  ...
                  <fer-select
                    ...
                    showSearch
                    allowClear
                    :disabled="ferKey === 'table'"
                    :showArrow="ferKey !== 'table'"
            ...
            <div v-else>
              <a-form-item>
                ...
                <fer-input
                  v-model:value="activeData.__vModel__"
                  placeholder="请输入数据库字段"
                  :disabled="ferKey === 'table' || !!formInfo.isKit"
                  @change="onInputFieldChange($event, config.formId, config.parentVModel)" />
              </a-form-item>
            </div>
          </div>
          <a-form-item label="控件标题" v-if="!layoutList.includes(ferKey) || ferKey === 'table'">
            <fer-i18n-input v-model:value="activeData.__config__.label" v-model:i18n="activeData.__config__.labelI18nCode" placeholder="请输入" />
          </a-form-item>
          <a-form-item label="显示标题" v-if="!layoutList.includes(ferKey)">
            <a-switch v-model:checked="activeData.__config__.showLabel" />
          </a-form-item>
          <a-form-item label="显示标题" v-if="ferKey === 'table'">
            <a-switch v-model:checked="activeData.__config__.showTitle" />
          </a-form-item>
          <a-form-item label="卡片标题" v-if="ferKey === 'card'">
            <fer-i18n-input v-model:value="activeData.header" v-model:i18n="activeData.headerI18nCode" placeholder="请输入" />
          </a-form-item>
          <a-form-item label="标题提示" v-if="hasTipLabel(ferKey)">
            <fer-i18n-input v-model:value="activeData.__config__.tipLabel" v-model:i18n="activeData.__config__.tipLabelI18nCode" placeholder="请输入" />
          </a-form-item>
          <a-form-item label="占位提示" v-if="activeData.placeholder !== undefined && !systemList.includes(ferKey)">
            <fer-i18n-input v-model:value="activeData.placeholder" v-model:i18n="activeData.placeholderI18nCode" placeholder="请输入" />
          </a-form-item>
          <div v-if="ferKey === 'table'">
          ...
          <a-form-item label="显示内容" v-if="ferKey === 'currOrganize'">
          ...
          <div v-if="![...layoutList, ...noVModelList].includes(ferKey) && ferKey !== 'input'">
            <a-form-item label="是否禁用" v-if="!systemList.includes(ferKey)">
              <a-switch v-model:checked="activeData.disabled" />
            </a-form-item>
            <a-form-item label="是否必填" v-if="!systemList.includes(ferKey)">
          ...
          <a-form-item label="是否隐藏" v-if="['relationFormAttr', 'popupAttr', 'calculate'].includes(ferKey) && activeData.isStorage">
            <a-switch v-model:checked="activeData.__config__.noShow" />
          </a-form-item>
...
<script lang="ts" setup>
  ...
  import { reactive, toRefs, computed, ref, unref, watch, onMounted, nextTick, useAttrs } from 'vue';
  import { ScrollContainer } from '@/components/Container';
  // import FieldModal from './components/FieldModal.vue';
  import FormScript from './components/FormAttrPane/components/FormScript.vue'; // 发生变化时：change
  import StylePane from './components/StylePane.vue';
  import FormAttrPane from './components/FormAttrPane/index.vue';
  import * as RightComp from './rightComponents';
  import { useModal } from '@/components/Modal';
  import { useGeneratorStore } from '@/stores/modules/generator';
  ...
  const getRightComp = computed(() => {
    ...
    if (orgList.includes(unref(ferKey))) return RightComp.ROrgRight;
    if (unref(ferKey) === 'popupTableSelect') return RightComp.RPopupSelect;
  ...
  const drawingOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && data.__config__.ferKey !== 'table') {
  ...
  watch(
    ...
    val => {
      ...
      if (!val.__config__.tableName && val.__config__.ferKey !== 'table') {
  ...
  function onChildTableFieldChange(val, formId) {
    ...
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        ...
        if (config.ferKey === 'table' && config.tableName === props.activeData.__config__.relationTable) {
        ...
        if (config && config.ferKey != 'table' && config.children && Array.isArray(config.children)) {
  ...
  function onInputFieldChange(val, formId, parentVModel) {
    ...
    if (parentVModel) {
      const loop = (data, parent?) => {
        ...
        if (Array.isArray(data)) data.forEach(d => loop(d, parent));
        if (parent?.__config__?.ferKey == 'table') {
          if (
            data.__vModel__ &&
      ...
      const loop = (data, parent?) => {
        if (!data) return;
        if (data.__config__?.ferKey !== 'table' && data.__config__?.children && Array.isArray(data.__config__.children)) {
          loop(data.__config__.children, data);
        }
        if (Array.isArray(data)) data.forEach(d => loop(d, parent));
  ...
  function onStorageChange(val) {
    if (val == 0) {
      ...
      if (!unref(getHasTable)) {
        props.activeData.__vModel__ = toggleVmodelCase(`${props.activeData.__config__.ferKey}Field${props.activeData.__config__.formId}`);
      } else {
  ...
  function onChildTableNameChange(tableName, formId) {
    ...
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        const e = list[i];
        const config = e.__config__;
        if (config.ferKey === 'table' && config.tableName === tableName && config.formId !== formId) {
          boo = true;
          break;
        }
        if (config && config.ferKey != 'table' && config.children && Array.isArray(config.children)) {
  ...
  function setDefaultOptions() {
    ...
    if ((props.activeData.__vModel__ === undefined && props.activeData.isStorage != 1) || props.activeData.__config__.ferKey === 'table') return;
    if (!props.activeData.__config__.tableName || props.activeData.__config__.tableName === unref(getMainTable)) {
      ...
      if (!list.length) {
        ...
        let options = item.fields.map(o => ({
          ...o,
          realField: 'fer_' + props.activeData.__config__.tableName + '_fer_' + o.field,
        }));
  ...
  function hasTipLabel(ferKey) {
    return ![...layoutList, ...systemList].includes(ferKey) || ['table', 'card', 'billRule'].includes(ferKey);
  }
  ...
  onMounted(() => {
    ...
    if (!props.activeData.__config__.tableName && props.activeData.__config__.ferKey !== 'table') {
      props.activeData.__config__.tableName = unref(getMainTable);
    }
    setDefaultOptions();
  });
