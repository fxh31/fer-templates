    ...
    setup(props, { attrs }) {
      ...
      const layouts = {
        colFormItem(element, index, parent) {
          ...
          if (config.ferKey == 'location') element.useAutoLocation = false;
          const Item =
            props.showType === 'app' && !useCompList.includes(config.ferKey) ? (
        ...
        rowFormItem(element, index, parent) {
          ...
          if (config.ferKey === 'row' || (props.showType === 'app' && config.ferKey === 'card')) {
            return (
              <a-col
                span={props.showType === 'app' ? 24 : config.span}
                data-draggable={true}
                ...
                <a-row class={className + ' drawing-row-item-row'}>
                  <span class="component-name">{config.ferKey === 'card' ? config.label : config.componentName}</span>
                  <a-col>
