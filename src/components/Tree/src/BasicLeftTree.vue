<template>
  <div :class="[bem(), 'h-full']">
    ...
    <div>
      ...
      <div :class="bem('search')" v-else>
        <a-select v-model:value="searchType" style="width: 80px" :options="options" @change="changeType"></a-select>
        <InputSearch :placeholder="t('common.leftTreeSearchText')" allowClear v-model:value="searchValue" />
      </div>
    </div>
    <BasicTree
      ...
      :filterFn="!showSearch ? filterFn : null"
      :loading="loading">
...
<script lang="ts" setup>
  ...
  function filterFn(searchValue, node) {
    return node[searchType.value]?.includes(searchValue);
  }
</script>
<style lang="less">
