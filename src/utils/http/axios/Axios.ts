...
export class VAxios {
  ...
  /**
   * @description:  Create axios instance 创建新的 axios 实例，用于配置连接不同后端
   */
  ...
  /**
   * @description:  File Upload 针对文件上传做处理
   */
  ...
  // 配合 qs 对 post、put、delete 等请求的 data 进行编码处理（与后端配合接取对应字段）
  ...
  request<T = any>(config: AxiosRequestConfig, options?: RequestOptions): Promise<T> {
    ...
    if (config.cancelToken) {
      conf.cancelToken = config.cancelToken; // cancelToken 如果被深拷贝，会导致最外层无法使用cancel方法来取消请求
    }
    ...
    if (beforeRequestHook && isFunction(beforeRequestHook)) {
      conf = beforeRequestHook(conf, opt); // 请求之前处理 config
    }

    conf = this.supportFormData(conf); // 格式化 data
